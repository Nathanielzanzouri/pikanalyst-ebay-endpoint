...
function buildPrompt(titles) {
  return [
    ...
    '- query construction is CATEGORY-DEPENDENT:',
    '  • For sneaker / apparel / bag → brand + model + colorway/edition ONLY. Do NOT include the SKU. Mainstream retailers (Foot Locker, Zalando, JD Sports, Courir, Sarenza, Citadium) use marketing names in product titles, not raw codes — adding the SKU pushes their pages out of Google Shopping results and leaves only specialized resellers. The colorway name alone disambiguates.',
    '  • For toy / watch / electronics → brand + model + variant + SKU. The SKU is how everyone searches for these (Lego "75192", Rolex "16610LN", iPhone model numbers) and mainstream retailers DO put it in their titles.',
    '  • For "other" / unknown → default to including the SKU when present (safer to be over-precise than under).',
    '- NEVER include in any query: retailer names, generic product nouns (sneakers, baskets, chaussures, schuhe, scarpe), size or condition tokens, gender markers (Wmns/Men/Femme).',
    '- Examples by category:',
    '  • sneaker → "Nike Air Heights Barely Rose"            (NO sku)',
    '  • sneaker → "adidas Samba OG Cloud White Core Black"  (NO sku)',
    '  • sneaker → "Converse Chuck Taylor Coca-Cola Black"   (NO sku, collab name disambiguates)',
    '  • bag     → "Louis Vuitton Speedy 30 Monogram"        (NO sku)',
    '  • apparel → "Stussy 8 Ball Hoodie Black"              (NO sku)',
    '  • toy     → "Lego Star Wars UCS Millennium Falcon 75192"  (WITH sku — that\'s how it\'s searched)',
    '  • watch   → "Rolex Submariner 16610LN"                (WITH sku — the reference IS the name)',
    '  • electronics → "Apple iPhone 15 Pro 256GB Natural Titanium MTU63LL/A"   (WITH sku when material disambiguator)',
    '- sku: the product code (e.g. Nike "IB8873-666", adidas "H03474", Lego "75192", Rolex "16610LN", LV "M41526") if it appears in at least one title; else null. Always extracted for the schema, even when not used in the query.',
